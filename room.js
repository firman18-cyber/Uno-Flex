...
// ==========================================
const MAX_PLAYERS = 8; // SAMAKAN DENGAN ANGKA DI APP.JS
// ==========================================
...
let isHost = false; let myIsActive = true; 
...
document.getElementById('my-card-trigger').addEventListener('click', () => { update(ref(db, `rooms/${currentRoom}/players/${myPlayerId}`), { isActive: !myIsActive }); });
...
onValue(ref(db, `rooms/${currentRoom}`), (snapshot) => {
    ...
    renderPlayers(players, data.hostId); checkAllRedRule(players);
});
...
function renderPlayers(players, hostId) {
    ...
    if (me) {
        ...
        const card = document.getElementById('my-power-card'); const label = document.getElementById('my-card-status');
    ...
    opps.forEach((p, index) => {
        ...
        const oppHtml = `<div class="opp-box"><div class="opp-avatar ${ringClass}">${isHostIcon}<img src="${getAvatarUrl(p.name)}" alt="Avatar"></div><div class="opp-name">${escapeHTML(p.name)}</div><div class="opp-minicard ${miniClass}"><svg viewBox="0 0 24 24" width="14" height="14">${svgIcon}</svg></div></div>`;
...
function checkAllRedRule(players) {
    const arr = Object.values(players); if (arr.length === 0) return;
    if (arr.every(p => p.isActive === false) && !isResetting) {
        isResetting = true; showToast("Semua Merah! Reset Otomatis...");
        if (isHost) {
            setTimeout(() => {
                const updates = {}; arr.forEach(p => updates[`rooms/${currentRoom}/players/${p.id}/isActive`] = true);
